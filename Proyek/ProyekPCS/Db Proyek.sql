...
CREATE OR REPLACE TRIGGER AUTOGEN_HSERVICE
BEFORE INSERT 
ON H_SERVICE
FOR EACH ROW
DECLARE
	...
	CTR NUMBER;
BEGIN
	SELECT COUNT(ID_SERVICE) INTO CTR FROM H_SERVICE;
	AUTOGEN := 'HS' || LPAD(CTR+1,4,'0');
	:NEW.ID_SERVICE := AUTOGEN;
	
END;
/
SHOW ERR;

CREATE OR REPLACE TRIGGER AUTOGEN_HJUAL
BEFORE INSERT 
ON H_JUAL
FOR EACH ROW
DECLARE
	...
	CTR NUMBER;
BEGIN
	SELECT COUNT(ID_JUAL) INTO CTR FROM H_JUAL;
	AUTOGEN := 'HJ' || LPAD(CTR+1,4,'0');
	:NEW.ID_JUAL := AUTOGEN;
...
INSERT INTO H_JUAL VALUES('',TO_DATE('27/05/2019','DD/MM/YYYY'),175000,'KE0002','LS0001');
